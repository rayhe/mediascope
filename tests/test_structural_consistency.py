...
class TestFramingDeviceTypeCount:
    ...
    EXPECTED_TOTAL = 80
    EXPECTED_PATTERN_MATCHED = 74  # core + extended (in _DEVICE_PATTERNS)
...
class TestTopicBucketConsistency:
    ...
    def test_topic_count_in_code(self):
        """Code should define exactly 27 topic buckets."""
        from mediascope.analyze.topics import TOPIC_KEYWORDS
        assert len(TOPIC_KEYWORDS) == 27, (
            f"Expected 27 topic buckets, got {len(TOPIC_KEYWORDS)}.\n"
    ...
    def test_methodology_topic_count(self):
        """METHODOLOGY.md must say 27 topic buckets."""
        doc = (_REPO_ROOT / "docs" / "METHODOLOGY.md").read_text()
        assert "27 topic buckets" in doc, (
            "METHODOLOGY.md topic count is stale. Should be 27."
        )

    def test_agent_guide_topic_count(self):
        """AGENT_GUIDE.md must list 27 topic buckets."""
        doc = (_REPO_ROOT / "docs" / "AGENT_GUIDE.md").read_text()
        assert "27 topic buckets" in doc, (
            "AGENT_GUIDE.md topic count is stale. Should be 27."
        )

    def test_architecture_topic_count(self):
        """ARCHITECTURE.md must say 27 topic buckets."""
        doc = (_REPO_ROOT / "docs" / "ARCHITECTURE.md").read_text()
        assert "27 topic buckets" in doc, (
            "ARCHITECTURE.md topic count is stale. Should be 27."
        )
...
class TestCrossReferenceConsistency:
    ...
    *secondary* references — tables, comparison sections, and inline mentions
    ...
    def test_readme_test_topics_description_says_25(self):
        """README.md test_topics.py description must reference 27 topic buckets."""
        doc = (_REPO_ROOT / "README.md").read_text()
        ...
        assert claimed == 27, (
            ...
            f"should be 27. The count is stale."
        )
    ...
    def test_readme_topic_count_in_description(self):
        ...
        assert not stale_refs, (
            ...
            f"Should be 'all 27' — 27 topic buckets defined in code."
        )
...
class TestAdversarialDeviceListConsistency:
    ...
    EXPECTED_TOTAL_PATTERNS = 451  # sum(len(v) for v in _DEVICE_PATTERNS.values())
