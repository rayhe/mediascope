"""
Type E #506 - Podcast Sentiment Tracking: Twenty-Fourth Verification Cycle Sep 4 00:00 PDT
Guilty Feminist 498 Hold No 499 as of 00:00 (zeno.fm opened this run, 24th cycle) +
EHE 25-Day Hold, No New Vertical, Six-Vertical Count Standing +
Attention Sphere 24th No-Match (nonprofit-org corroboration stays secondary-only)
"""
import re
...
def get_506_block():
    ...
    m = re.search(r"^## Iteration #506", text, re.MULTILINE)
    assert m, "Iteration #506 block not found in podcast-sentiment.md"
    rest = text[m.end():]
...
class TestIterationNumberAndRotation:
    ...
    def test_twentyfourth_cycle_label(self):
        block = get_506_block()
        lower = block.lower()
        assert "twenty-fourth" in lower

    def test_extends_501(self):
        block = get_506_block()
        lower = block.lower()
        assert "501" in block
...
class TestAttentionSphere:
    ...
    def test_nonprofit_corroboration_stays_secondary(self):
        # New this cycle: Purposeful Empathy result corroborates the nonprofit
        # identification but must not be elevated above secondary-only.
        block = get_506_block()
        lower = block.lower()
...
class TestStatisticalHygiene:
    ...
    def test_no_em_dashes(self):
        block = get_506_block()
        assert "—" not in block, "Em dash found in #506 block"

    def test_https_only(self):
        block = get_506_block()
        urls = re.findall(r'https?://[^\s)"]+', block)
        assert urls, "Expected URLs in #506 block"
        for u in urls:
...
class TestNoveltyAndDuplicatePrevention:
    def test_distinct_from_501(self):
        block = get_506_block()
        assert "501" in block
        lower = block.lower()
        assert "distinct from" in lower or "extends #501" in lower
...
class TestIterationLog:
    def test_iteration_log_506_exists(self):
        log_text = LOG_PATH.read_text(encoding="utf-8")
        assert "#506" in log_text
        assert "Type E" in log_text
        assert "2026-09-04 00:00 PDT" in log_text
    ...
    def test_log_newest_first_relative(self):
        ...
        log_text = LOG_PATH.read_text(encoding="utf-8")
        idx_506 = log_text.find("#506 Type E")
        idx_505 = log_text.find("#505 Type D")
        assert idx_506 != -1 and idx_505 != -1
        assert idx_506 < idx_505, "#506 entry must precede #505 (newest-first)"
