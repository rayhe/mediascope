"""
Type E #496 - Podcast Sentiment Tracking: Twenty-Second Verification Cycle Sep 3 14:00 PDT
Guilty Feminist 498 Hold No 499 as of 14:00 (zeno.fm opened this run, 22nd cycle) +
EHE 24-Day Hold, No New Vertical, Six-Vertical Count Standing +
Attention Sphere 22nd No-Match
"""
import re
...
def get_496_block():
    ...
    m = re.search(r"^## Iteration #496", text, re.MULTILINE)
    assert m, "Iteration #496 block not found in podcast-sentiment.md"
    rest = text[m.end():]
...
class TestIterationNumberAndRotation:
    ...
    def test_twentysecond_cycle_label(self):
        block = get_496_block()
        lower = block.lower()
        assert "twenty-second" in lower

    def test_extends_491(self):
        block = get_496_block()
        lower = block.lower()
        assert "491" in block
...
class TestStatisticalHygiene:
    ...
    def test_no_em_dashes(self):
        block = get_496_block()
        assert "\u2014" not in block, "Em dash found in #496 block"

    def test_https_only(self):
        block = get_496_block()
        urls = re.findall(r'https?://[^\s\)\"]+', block)
        assert urls, "Expected URLs in #496 block"
        for u in urls:
...
class TestNoveltyAndDuplicatePrevention:
    def test_distinct_from_491(self):
        block = get_496_block()
        assert "491" in block
        lower = block.lower()
        assert "distinct from" in lower or "extends #491" in lower
...
class TestIterationLog:
    def test_iteration_log_496_exists(self):
        log_text = LOG_PATH.read_text(encoding="utf-8")
        assert "#496" in log_text
        assert "Type E" in log_text
        assert "2026-09-03 14:00 PDT" in log_text
    ...
    def test_log_newest_first_relative(self):
        ...
        log_text = LOG_PATH.read_text(encoding="utf-8")
        idx_496 = log_text.find("#496 Type E")
        idx_495 = log_text.find("#495 Type D")
        assert idx_496 != -1 and idx_495 != -1
        assert idx_496 < idx_495, "#496 entry must precede #495 (newest-first)"
