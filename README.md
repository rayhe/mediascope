...
MediaScope has **2301 tests** across 102 test files, each covering a different analytical capability:
...
- **Sentiment correction coverage:** Two article genres lack correction paths — financial journalism (VADER inflated by investment vocabulary) and structural-split articles (adversarial lead + promotional quote tail). Both are fully documented with proposed solutions in [METHODOLOGY.md §16](docs/METHODOLOGY.md#16-financial-journalism-sentiment-bias). The 11 existing paths (A–K) each address a distinct VADER failure mode; see [METHODOLOGY.md §9](docs/METHODOLOGY.md#9-framing-aware-tone-correction) for the full decision tree with trigger thresholds and blend formulas.
