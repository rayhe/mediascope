...
Ships with verified career data for **192 journalists** across 360+ publications (595 auto-detected migrations). Notable high-value migrations include:
...
| [FRAMING_REFERENCE.md](docs/FRAMING_REFERENCE.md) | **Quick-reference card for all 91 framing device types** — scannable lookup during article analysis |
...
| [`careers_demo.py`](examples/careers_demo.py) | Editorial Histories module: career timelines for 192 journalists, 595 auto-detected migrations, DiD natural experiment setup, and notable career pipelines |
| [`topic_classification_demo.py`](examples/topic_classification_demo.py) | **NEW:** 28-bucket topic classification system — demonstrates single-article classification, multi-topic overlap, genre detection via topic confidence, and the full bucket reference with adjacency warnings for commonly confused pairs |
...
MediaScope has **2147 tests** across 91 test files, each covering a different analytical capability:
...
| `test_structural_consistency.py` | 96 | Structural consistency guards: framing device type count (91 total = 84 pattern-matched + 7 structural), total regex pattern count (519 patterns across all pattern-matched types), `precedent_analogy` exists, `__main__.py` entry point works, doc counts match across METHODOLOGY.md/ARCHITECTURE.md/AGENT_GUIDE.md/CLI/README (framing types, banned phrases, Advance/Reddit voting power, topic bucket counts), test file listing completeness (every test file on disk appears in ARCHITECTURE.md and README.md, no phantoms, header count matches), README/ARCHITECTURE total test count header guards (validates pytest-collected count including parametrize expansions), stale voting power purge guards across all doc files, cross-reference consistency guards (no stale framing taxonomy count references in any doc, README test descriptions reference correct topic bucket and framing device counts, parenthetical device count annotations in diagrams), inline topic list validation (ARCHITECTURE.md, AGENT_GUIDE.md, METHODOLOGY.md topic names match code), quality standards banned phrase count and completeness guards, framing.py docstring count validation (pattern-matched and total counts match code), METHODOLOGY.md device table completeness (Extended and Structural tables contain all code device types), METHODOLOGY.md intro tier count guard (91/10/74/7 matches code), METHODOLOGY.md §17 annotated article count and publication count guards (corpus of N real articles and N distinct publications match actual files on disk), adversarial device type list consistency (METHODOLOGY.md, QUALITY_STANDARDS.md, AGENT_GUIDE.md, and example demo scripts enumerate all 27 adversarial types from sentiment.py), ARCHITECTURE.md test_topics bucket count guard, ARCHITECTURE.md device name list completeness (Core and Extended inline lists enumerate all device types from code), AGENT_GUIDE.md framing tier count guard (91/10/74/7 matches code), correction path documentation completeness (all 10 sentiment correction paths A-J documented in METHODOLOGY.md, ARCHITECTURE.md, AGENT_GUIDE.md, README.md, and example demo scripts with summary table), journalist count consistency (YAML journalist count matches README.md, EDITORIAL_HISTORIES.md, and careers_demo.py; multi-pub count matches docs; CareerTracker loads all journalists without error; stale journalist and multi-pub count purge across all EDITORIAL_HISTORIES.md references), migration count guards (README.md careers_demo migration count and EDITORIAL_HISTORIES.md migration count both match CareerTracker computed count), publication count floor guards (README.md and EDITORIAL_HISTORIES.md publication count floor stays within 20 of actual unique slug count), entity cluster consistency (METHODOLOGY.md §15 cluster count matches code, table completeness with no missing/phantom clusters, alias count accuracy), annotated article count guard (QUALITY_STANDARDS.md article count matches examples/sample_output/*_analysis.md), same-event cluster count guard (QUALITY_STANDARDS.md cluster count matches Tier 1 + Tier 2 table rows), example demo adversarial type set completeness (framing_correction_demo.py and sarcastic_editorial_demo.py inline adversarial_types match code), emotional language term count consistency (QUALITY_STANDARDS.md bold count matches actual EMOTIONAL_LANGUAGE list length in sentiment.py), stale device count purge (no doc references historic device type counts that aren't the current total, including parenthetical annotations) |
