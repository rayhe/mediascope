...
MediaScope detects 113 framing device types, organized into three tiers: core devices (10 pattern-matched types covering fundamental editorial techniques), extended devices (96 added from real-article analysis), and structural devices (7 detected via post-pass heuristics rather than simple pattern matching).
...
| **Category Contamination** | One company's reputational damage contaminates an entire product category, forcing competitors to delay, redesign, or explicitly distance themselves. Converts company-specific failures into category-level stigma that ANY new entrant must address. Distinct from competitive_positioning (company-to-company comparison), competitive_displacement (filling a vacuum), and glasshole_revival (reviving a pejorative): category contamination frames the structural CONSEQUENCE of stigma on competitors' timelines and product decisions | "[Company A]'s reputation directly influences [Company B]'s timeline"; "don't want to ship a device that invites the same backlash"; "poisoned the well"; "wants none of it [privacy baggage]"; "How do you put cameras on someone's face without sparking outrage?" | WebProNews (Jul 27, 2026) — "Meta's reputation directly influences Apple's timeline. Engineers don't want to ship a device that invites the same backlash." Also: AppleInsider (Jul 26, 2026) — "poisoning the well when it comes to consumers"; TechSpot (Jul 27, 2026) — privacy concerns delaying Apple's timeline |
...
| **Framing device count** | Total devices from the 113-type taxonomy (§4) | Framing density — how many editorial techniques are deployed |
...
| **Google** | 13 | Alphabet, Google, YouTube, DeepMind, Waymo, Sundar Pichai, Google Messages, Android XR (+5 more) |
| **Apple** | 12 | Apple, iPhone, iPad, Tim Cook, John Ternus, Apple Intelligence, iMessage (+5 more) |
| **Amazon** | 14 | Amazon, AWS, Alexa, Jeff Bezos, Andy Jassy, Amazon Web Services, Trainium, Trainium2, Trainium3, Inferentia, Echo Frames (+3 more) |
...
| **BBC** | 2 | BBC, British Broadcasting Corporation |
| **Recording Wearable Devices** | 8 | Amazon Bee, Bee Pioneer, Plaud, Notepin, Notepin S, Humane AI Pin, Omi, Compass |
...
| **Smart Glasses Competitors** | 15 | Gentle Monster, XREAL, Even Realities, Halo, Solos, Brilliant Labs, Halliday, RayNeo, Rokid, VITURE, INMO, Meizu StarV, HTC VIVE Ego, HTC VIVE Eagle, Lucyd |
...
MediaScope's analytical methods — framing device taxonomy, sentiment correction paths, source stance analysis, and same-event comparison methodology — are all grounded in a manually annotated corpus of **206 real articles**. Every framing device type was discovered from a real article, every correction path was triggered by a real VADER failure, and every analytical method is validated against real editorial output.
...
The corpus spans **50 distinct publications** across 5 editorial modes:
...
| **The Register** | 4 | Brain2Qwerty BCI research, Muse Image |
...
Every one of the 113 framing device types was discovered from a specific article in the corpus or from the broader analysis pipeline. The METHODOLOGY.md §4 extended device table documents the discovery article for each type. Key discovery clusters:
