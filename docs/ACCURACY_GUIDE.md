...
This creates **polarity inversion**: VADER assigns a positive raw score to an article whose editorial stance is clearly negative. Of the 206 manually annotated articles in MediaScope's corpus, **at least 21 exhibit documented polarity inversion** where VADER's raw score is positive (+0.30 to +0.99) but the true editorial tone is negative (−0.20 to −0.72).
...
From the 200 annotated articles:
...
> Verified score deltas from the 200-article annotated corpus. Each entry shows the raw VADER score, the corrected score (if correction fired), the manual assessment, and the residual error. Use this to calibrate expectations for each correction path.
>
