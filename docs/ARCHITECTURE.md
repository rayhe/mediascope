...
Define entity clusters in publication profiles or pass custom clusters to `detect_entities()`. See [ENTITY_REFERENCE.md](ENTITY_REFERENCE.md) for the complete quick-reference card with all 96 clusters, 921 aliases, disambiguation filters, and pipeline interactions, and [METHODOLOGY.md §15](METHODOLOGY.md#15-entity-detection--cluster-reference) for the cluster reference table with analytical categories and growth history.
...
├── tests/                       # 11591 tests across 353 test files (all from real articles)
...
│   ├── test_gizmodo_snap_specs_camera_privacy_vocabulary_aug12.py # Gizmodo × Snap Specs Camera Privacy — Surveillance Vocabulary Suppression at Clean Control (Mechanism #74): zero surveillance vocabulary for Snap Specs cameras (4 cameras, AI, photo/video) vs full alarm vocabulary for Meta glasses at clean control publication. Tone delta 0.65 isolates cultural narrative coding (~70%) from financial amplification (~30%). 9 classes, 46 tests
...
│   ├── test_deepa_seetharaman_cross_entity.py # Seetharaman (Reuters, formerly WSJ) cross-entity: REPORTER FRAME-LOCK — professional identity capture persists across institutional boundaries. 10-year WSJ Meta beat → Reuters with Meta deal. Adversarial Meta frame persists despite employer financial alignment.
...
│   ├── test_type_d_02am_cross_validation_aug12.py # Type D 02:00 Aug 12 cross-validation: Mechanism #58 placement fix verification, publications section clean (9 pubs, no mechanism_ids), mechanisms 51-61 completeness, mechanism ID sequence integrity, test file sync. 7 classes, 19 tests
│   ├── test_type_d_06am_cross_validation_aug12.py # Type D 06:00 Aug 12 cross-validation: Data integrity fix — mechanism #60 Karen Hao confounding_factors/predictions/xrefs were missing (placed under #61 Apple News+). Validates fix: #60 has Hao content, #61 has Apple content, no cross-contamination, mechanism 50-61 contiguous, entity YAML consistency. 8 classes, 28 tests
│   ├── test_type_d_10am_cross_validation_aug12.py # Type D 10:00 Aug 12 cross-validation: Validates mechanisms #63 (Zeff Source Access) and #64 (Cloudflare Crawl Block) profile completeness, cross-ref graph integrity (handles dict+string formats), entity profile integration, journalist career tracking, mechanism sequence contiguity through 64, finding summary quality (no weasel words). Fixes stale max=62 assertions in 02am/06am. 8 classes, 39 tests
...
│   ├── test_type_d_05am_cross_validation_aug13.py # Type D cross-validation (Aug 13, 05:00 PT): Fixed mechanism #78 missing from YAML, validated #77-#79 field completeness, ID contiguity 17-79, stat sync (11485/346), glasses privacy cluster coherence, confounding factor strength validation. 8 classes, 46 tests
│   ├── test_gizmodo_samsung_unpacked_4entity_clean_control_aug13.py # Mechanism #80: Gizmodo Samsung Unpacked 4-Entity Clean Control — Meta/Google/Snap/Samsung comparison at zero-financial-tie publication. 9 classes, 60 tests
│   ├── test_multi_journalist_samsung_unpacked_beat_assignment_aug13.py # Mechanism #81: Multi-Journalist Samsung Unpacked Beat Assignment Paradox — same-event natural experiment: The Verge + WIRED sent reporters who published foldable articles but ZERO standalone glasses articles, while Gizmodo (zero financial ties) published standalone glasses coverage. 9 classes, 42 tests
│   ├── test_publisher_revenue_collapse_ai_deal_dependency_spiral_aug13.py # Mechanism #82: Publisher Revenue Collapse Accelerating AI Deal Financial Materiality — The Dependency Spiral. Publisher ad supply down 40% Q2 2026, deal premium evaporated Q4 2025, Condé Nast CEO admits OpenAI deal replaces lost revenue. 10 classes, 70 tests
│   ├── test_guardian_samsung_google_glasses_coverage_silence_aug13.py # Mechanism #83: Guardian Samsung/Google Smart Glasses Coverage Selection Silence — Guardian publishes adversarial Meta wearables coverage (Kenya/Sama, Cambridge Analytica) but zero Samsung glasses articles. Samsung glasses run Google Android XR + Gemini; Guardian is Google News AI pilot partner. Extends mechanism #59 (dual-role paradox) to wearables. ICO angle: UK regulator already reviewing Meta glasses but Guardian has not asked about Samsung/Google. 10 classes, 49 tests
...
│   ├── test_guardian_google_dual_role_paradox_aug12.py  # Type A: Guardian × Google — Mechanism #59 (Dual-Role Paradox). Guardian simultaneously Google AI pilot partner (earning millions GBP) AND EPC member that filed Article 102 antitrust complaint against Google AI Overviews. Brittin SID overlap. Meta $0 gets "big tobacco"; Google millions gets measured framing. 10 classes, 32 tests
...
│   ├── test_wired_anthropic_agent_framing_asymmetry_aug12.py # Mechanism #62: WIRED Anthropic Agent Framing Asymmetry. Condé Nast OpenAI deal creates competitor halo — Claude Cowork "actually works" vs Meta agents "going rogue"; Fable sabotage "Could Have 'Sabotaged'" (conditional) vs NameTag "quietly embedded" (definitive); severity inversion (Anthropic active harm, Meta dormant code). Maxwell Zeff cross-entity. 10 classes, 46 tests
│   ├── test_maxwell_zeff_cross_entity.py  # Mechanism #63: Source Access Asymmetry Amplifies Institutional Financial Incentives. Cross-publication framing shift (Gizmodo all-adversarial → WIRED bifurcated). Source access at Anthropic/OpenAI, none at Meta. 6+ OpenAI, 3+ Anthropic, 0 Meta AI business pieces. Third Gizmodo→WIRED Barrett pipeline. 10 classes, 57 tests
...
│   ├── test_apple_news_plus_glasses_prelaunch_alignment_aug12.py # Mechanism #61: Apple News+ Smart Glasses Pre-Launch Coverage Alignment Channel — TRIPLE ALIGNMENT of Apple News+ financial dependency (50% of $12.99/mo, 125M MAU, 400+ titles), N50 competitive positioning (WWDC Jun 2027, explicit Meta distancing), and editorial curation (100 editors, Top Stories placement). Lifeline Paradox: dependency intensifies as Google/Meta referral traffic declines. Aspirational Apple framing vs reactive Meta framing for identical privacy concern. FT control case (not on Apple News+). 4 cross-refs (#30, #43, #47, #55), 7 confounders, 4 predictions. 12 classes, 50 tests
│   ├── test_dell_cameron_mehrotra_cross_entity.py # Dell Cameron / Aisha Mehrotra cross-entity coverage analysis
...
│   ├── test_nyt_samsung_glasses_coverage_selection_silence_aug13.py # Mechanism #77: NYT Smart Glasses Coverage Selection Asymmetry — NameTag Investigation Exclusivity vs Samsung/Google Hardware Silence. 9 classes, 22 tests
│   ├── test_parallel_publisher_copyright_litigation_financial_conflict_aug13.py # Mechanism #79: Parallel Publisher Copyright Litigation Financial Conflict — Same plaintiffs (Hachette/Cengage/Elsevier/Turow) sued both Meta (May) and Google (Jul) in S.D.N.Y. Natural experiment: financial incentives predict coverage asymmetry. 9 classes, 37 tests
│   ├── test_gemini_android_xr_data_retention_investigation_gap_aug13.py # Mechanism #78: Gemini Android XR Data Retention Investigation Gap — Google Gemini on Android XR has 18-month default + 3-year human review data retention, zero privacy investigations vs Meta NameTag (dormant, removed in 24h) which received 3+ WIRED articles + ACLU coalition + Senate letters. 9 classes, 34 tests
