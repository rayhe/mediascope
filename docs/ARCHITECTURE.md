...
Raw Text
  ...
  └── (Optional) LLM score ─────┘          │
                                 ...
                                 (110 device types, 748 compiled regex)
                                           │
                                           ▼
...
### `framing.py`
- **110 framing device types** organized in three tiers:
...
Define entity clusters in publication profiles or pass custom clusters to `detect_entities()`. See [ENTITY_REFERENCE.md](ENTITY_REFERENCE.md) for the complete quick-reference card with all 93 clusters, 898 aliases, disambiguation filters, and pipeline interactions, and [METHODOLOGY.md §15](METHODOLOGY.md#15-entity-detection--cluster-reference) for the cluster reference table with analytical categories and growth history.
...
│   └── sample_output/       # 197 annotated real-article analyses (see METHODOLOGY.md §17)
├── tests/                       # 3078 tests across 139 test files (all from real articles)
