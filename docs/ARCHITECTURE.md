...
The toolkit uses a multi-layer correction pipeline with **11 distinct paths (A–K)** that addresses known VADER/TextBlob blind spots when scoring editorial prose:
...
Raw Text
  ...
  └── (Optional) LLM score ─────┘          │
           ...
           ┌────────┬──────────┬────┼────┬──────────┐
           ▼        ▼          ▼    ▼    ▼          ▼
        Path A   Path B    Path C  Path E  Path D  Path F  Path H  Path I  Path J  Path K
        Full     Amplify   Anchor  Mil.   Sardonic Contra- Sarcastic Consumer Expert Sarcastic
        correct  understat embed   techno mocking  dictory editorial critique struct. rejection
                                   optim          review
                                           │
                                           ▼
...
### `framing.py`
- **98 framing device types** organized in three tiers:
...
Define entity clusters in publication profiles or pass custom clusters to `detect_entities()`. See [ENTITY_REFERENCE.md](ENTITY_REFERENCE.md) for the complete quick-reference card with all 85 clusters, 835 aliases, disambiguation filters, and pipeline interactions, and [METHODOLOGY.md §15](METHODOLOGY.md#15-entity-detection--cluster-reference) for the cluster reference table with analytical categories and growth history.
...
│   └── sample_output/       # 160 annotated real-article analyses (see METHODOLOGY.md §17)
