...
```json
{
    ...
    "description": "Detect editorial framing devices in article text. Returns a list of FramingDevice objects with device_type, evidence_text, and character offsets. Detects 101 device types across three tiers: core (10 pattern-matched), extended (84 from real-article analysis), and structural post-pass (7 heuristic-based).",
    "parameters": {
        "type": "object",
        "properties": {
            "text": {
...
The pipeline evaluates 12 paths (A–L) in priority order — the first match fires:
